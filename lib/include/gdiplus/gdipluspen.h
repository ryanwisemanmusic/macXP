...
public:
		...
		nativePen(NULL), lastStatus(Ok)
	{
		lastStatus = DllExports::GdipCreatePen1(
				color.GetValue(), width, (GpUnit) UnitWorld,
				&nativePen);
	}
		...
		nativePen(NULL), lastStatus(Ok)
	{
		lastStatus = DllExports::GdipCreatePen2(
				...
				width, (GpUnit) UnitWorld, &nativePen);
	}
	~Pen()
	{
	...
	PenAlignment GetAlignment() const
	{
		...
		updateStatus(DllExports::GdipGetPenMode(nativePen, (GpPenAlignment*)&result));
		return result;
	}
	...
	DashStyle GetDashStyle() const
	{
		...
		updateStatus(DllExports::GdipGetPenDashStyle(
				nativePen, (GpDashStyle*)&result));
		return result;
	}
	LineCap GetEndCap() const
	{
		LineCap result = LineCapFlat;
		updateStatus(DllExports::GdipGetPenEndCap(nativePen, (GpLineCap*)&result));
		return result;
	}
			...
			MatrixOrder order = MatrixOrderPrepend)
	{
		return updateStatus(DllExports::GdipTranslatePenTransform(
				nativePen, dx, dy, (GpMatrixOrder) order));
	}
