...
#else
/* Unknown processor type: provide a minimal generic CONTEXT so headers
   can be parsed on non-Windows hosts (IntelliSense/Clang). This is a
   conservative placeholder used only when no architecture branch matched. */
typedef struct _CONTEXT {
	ULONG ContextFlags;
} CONTEXT;
#endif
